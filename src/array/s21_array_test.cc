...
TEST(arrayTest, DefaultConstructor_Emptyarray) {
  ...
  EXPECT_EQ((const int)v.size(), (const int)5);
}
...
TEST(array_test, end) {
  ...
  EXPECT_EQ((int)vec1.size(), (int)vec2.size());
}

TEST(array_test, size) {
  s21::array<int, 3> vec1;
  std::array<int, 3> vec2 = {1, 2, 3};
  EXPECT_EQ((int)vec1.size(), (int)3);
  vec1 = {1, 2, 3};
  EXPECT_EQ(vec1.size(), vec2.size());
  EXPECT_EQ(vec1.size(), vec2.size());
}
